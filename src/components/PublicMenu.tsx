...
export default function PublicMenu({ onBack }: { onBack: () => void }) {
  ...
  function confirmOnWhatsapp(order: Order) {
    ...
    const message = `Olá! Acabei de fazer o pedido #${String(order.number).padStart(3, "0")} pelo cardápio da Braseira Pizza.\n\n${itemLines}\n\nTotal: ${money.format(order.total)}\nCliente: ${order.customerName}`;
    window.open(`https://wa.me/${phone}?text=${encodeURIComponent(message)}`, "_blank", "noopener,noreferrer");
  }

  return <div className="public-menu">
    <header className="public-header"><button className="back-admin" onClick={onBack}><ArrowLeft size={17} /> Administração</button><div className="public-brand"><span><Flame size={22} /></span><div><strong>Braseira Pizza</strong><small>{online ? `Cardápio online • ${storeOpen ? "Loja aberta" : "Loja fechada"}${minimumOrder ? ` • Mín. ${money.format(minimumOrder)}` : ""}` : "Feita no fogo. Feita para você."}</small></div></div><button className="cart-button" onClick={() => setCartOpen(true)}><ShoppingBag size={19} /><span>{cartCount}</span><b>{money.format(cartTotal)}</b></button></header>
    ...
    {completedOrder && <div className="menu-overlay"><div className="order-success"><CheckCircle2 size={58} /><small>PEDIDO RECEBIDO</small><h2>Pedido #{String(completedOrder.number).padStart(3, "0")}</h2><p>Seu pedido já foi enviado para a central da Braseira Pizza.</p><strong>{money.format(completedOrder.total)}</strong>{whatsapp && <button className="whatsapp-order" onClick={() => confirmOnWhatsapp(completedOrder)}><MessageCircle size={18} /> Confirmar pelo WhatsApp</button>}<button className="back-menu" onClick={() => setCompletedOrder(null)}>Voltar ao cardápio</button></div></div>}
  </div>;
}
